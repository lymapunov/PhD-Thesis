...
while t < tf:
    ...
    f = 1.0*(-delta*xp - alpha*x - beta*x**3 + gamm*np.sin(omega*t))
...
ax0.plot(t_, x_, linewidth=1, c='blue', label='Obtido')
...
ax1.plot(t_, [x_[i]-xd_[i] for i in range(len(x_))], linewidth=1, c='blue')
...
ax1.set_yticks([-0.2,0.0,0.2,0.4,0.6],[-0.2,0.0,0.2,0.4,0.6], fontsize=16)

ax2.plot(t_, u_, linewidth=1, c='blue')
...
plt.savefig('duffing_withoutANN-all perfect.pdf', bbox_inches='tight')
...
# while s < 5:
#     a = ativ(s,c,l)
#     d_.append(np.dot(w.T,a))
#     s_.append(s)
#     s += dt

# plt.figure(2)
# plt.plot(t_,d_est_,t_,y_)
# plt.figure(3)
# plt.plot(s_,d_)
