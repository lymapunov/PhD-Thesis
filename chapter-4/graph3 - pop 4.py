...
learn = 200
N_pop = 4
directory = 'data/modelo de massas_-fblann-case'+str(case)+'_learn'+"{:.2e}".format(learn)+'_pop'+str(N_pop)+'_data.txt'
...
ax3.add_line(l22)
ax3.annotate("\\textbf{Início da crise}\n\\textbf{em "+str(round(first_peak_3,3))+" s}", (cx2, cy2-0.3), color='black', weight='bold', fontsize=12, ha='center', va='center', zorder=3)
...
ax5.plot(t, u, linewidth=1, c='black')
ax5.set_title("Sinal de controle", fontsize=14, pad=4.5)
ax5.set_xlabel("Tempo [s]", fontsize=14)
ax5.set_ylabel("$J_2$ [$\mu$A]", fontsize=14)
ax5.set_xlim([0,90])
ax5.set_xticks([0,30,60,90],[0,30,60,90], fontsize=12)
ax5.set_ylim([-900,300])
ax5.set_yticks([-900,-600,-300,0,300],[-900,-600,-300,0,300], fontsize=12)

# Adjust layout to prevent overlap
plt.tight_layout()

plt.text(-68,3195,r'\textbf{(a.1)}',fontsize=14)
plt.text(38.5,3195,r'\textbf{(a.2)}',fontsize=14)
plt.text(-68,1755,r'\textbf{(a.3)}',fontsize=14)
plt.text(38.5,1755,r'\textbf{(a.4)}',fontsize=14)
plt.text(-13,305,r'\textbf{(b)}',fontsize=14)

plt.savefig('graph3-fblann-'+str(case)+'_pop'+str(N_pop)+'-pt.pdf', bbox_inches='tight')
